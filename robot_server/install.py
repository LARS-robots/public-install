...
Этот установщик:
 ...
 - устанавливает NetworkManager dispatcher hook для WiFi guard,
...
DISPATCHER_TARGET = Path("/etc/NetworkManager/dispatcher.d/90-ap-guard.sh")
...
def install_systemd_unit():
    ...
    try:
        ...
        # Fix the working directory to point to robot_server folder
        txt = txt.replace("WorkingDirectory=/home/pi/LARS", f"WorkingDirectory={INSTALL_DIR}/robot_server")
        
        # Fix PYTHONPATH to point to robot_server folder  
        txt = txt.replace("Environment=PYTHONPATH=/home/pi/LARS", f"Environment=PYTHONPATH={INSTALL_DIR}/robot_server")
        
        # Fix ExecStart to use correct module path (app.main:app instead of robot_server.app.main:app)
        txt = txt.replace("robot_server.app.main:app", "app.main:app")
        
        # Update python path
        venv_python = VENV_DIR / "bin" / "python"
        txt = txt.replace("/home/pi/LARS/venv/bin/python", str(venv_python))
        
        # Use consistent port (8000 instead of 8081)
        txt = txt.replace("--port 8081", "--port 8000")
...
def install_nm_dispatcher_hook():
    """Устанавливает NetworkManager dispatcher hook для WiFi guard механизма"""
    print("Installing NetworkManager dispatcher hook...")
    
    # Ищем исходный файл в нескольких возможных местах
    possible_sources = [
        INSTALL_DIR / "90-ap-guard.sh",
        INSTALL_DIR / "robot_server" / "scripts" / "90-ap-guard.sh",
        INSTALL_DIR / "scripts" / "90-ap-guard.sh"
    ]
    ...
    if not source_file:
        print("  ⚠️ 90-ap-guard.sh not found, skipping dispatcher hook installation")
        return False
    
    print(f"  Found dispatcher script at: {source_file}")
    
    # Проверяем, что это текстовый файл
    if not is_text_file(source_file):
        print(f"  ❌ Dispatcher script is not a text file: {source_file}")
        return False
    
    try:
        # Создаем директорию dispatcher.d если её нет
        dispatcher_dir = DISPATCHER_TARGET.parent
        run_cmd(["sudo", "mkdir", "-p", str(dispatcher_dir)], check=True)
        
        # Проверяем, существует ли уже файл
        if DISPATCHER_TARGET.exists():
            print(f"  ⚠️ Existing dispatcher hook found at {DISPATCHER_TARGET}, replacing...")
            run_cmd(["sudo", "rm", str(DISPATCHER_TARGET)], check=True)
        
        # Копируем файл
        run_cmd(["sudo", "cp", str(source_file), str(DISPATCHER_TARGET)], check=True)
        
        # Устанавливаем права на выполнение
        run_cmd(["sudo", "chmod", "+x", str(DISPATCHER_TARGET)], check=True)
        
        # Проверяем, что файл скопирован и исполняемый
        result = subprocess.run(["sudo", "ls", "-la", str(DISPATCHER_TARGET)], 
                              capture_output=True, text=True, check=False)
        if result.returncode == 0:
            print(f"  ✅ Dispatcher hook installed successfully:")
            print(f"    {result.stdout.strip()}")
        else:
            print(f"  ⚠️ Could not verify dispatcher hook installation")
        
        # Перезапускаем NetworkManager для применения изменений
        print("  Restarting NetworkManager to apply dispatcher changes...")
        run_cmd(["sudo", "systemctl", "restart", "NetworkManager"], check=False)
        
        return True
        
    except Exception as e:
        print(f"  ❌ Failed to install dispatcher hook: {e}")
        traceback.print_exc()
        return False
...
def check_installation():
    ...
    # Проверяем dispatcher hook
    if DISPATCHER_TARGET.exists():
        result = subprocess.run(["sudo", "test", "-x", str(DISPATCHER_TARGET)], check=False)
        if result.returncode == 0:
            print("✅ NetworkManager dispatcher hook installed and executable")
        else:
            print("⚠️ Dispatcher hook exists but not executable")
    else:
        print("❌ NetworkManager dispatcher hook not installed")
...
def main():
    ...
    for item in ["robot_server", "requirements.txt", "setup_wifi.py", "systemd", "VERSION", "90-ap-guard.sh"]:
        p = INSTALL_DIR / item
        if p.exists():
            if p.is_dir():
                shutil.rmtree(p)
    ...
    try:
        ...
        # Устанавливаем NetworkManager dispatcher hook
        if not install_nm_dispatcher_hook():
            print("  ⚠️ NetworkManager dispatcher hook not installed - WiFi guard may not work properly.")
        ...
        print("  sudo journalctl -u NetworkManager -f  # для мониторинга WiFi событий")
